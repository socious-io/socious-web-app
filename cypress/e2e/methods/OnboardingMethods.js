...
class OnboardingMethods{
    ...
    setEmail(onboardingEmail){
        OnboardingElements.elements.emailTxt().clear().type('testazintest60@outlook.com');
        return this        
    }
    ...
    assertValidationMsgForNullCountryAndCity(){
        OnboardingElements.elements.countryNullValMsg().should('contain','cannot be an empty field')
        OnboardingElements.elements.cityNullValMsg().should('contain','cannot be an empty field')
    }
    selectCountry(){
