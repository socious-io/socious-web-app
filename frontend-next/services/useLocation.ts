...
const useLocation = () => {
                ...
                request
                    .get(
                        `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${encodeURIComponent(
                            searchKeyword,
                        )}&types=%28countries%29&language=${language ?? 'en_US'}&key=${GOOGLE_API}`,
                    )
                    .then((response: any) => {
    ...
    const getCityByKeyword = (selectedCountry: any, searchKeyword: string | undefined) => {
        const language = 'en_US';
        return new Promise((resolve: (response: any) => void, reject) => {
            searchKeyword &&
                selectedCountry &&
                request
                    .get(
                        `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${encodeURIComponent(
                            searchKeyword,
                        )}&types=%28cities%29&components=country:${selectedCountry?.country_code}&language=${
                            language ?? 'en_US'
                        }&key=${GOOGLE_API}`,
                    )
                    .then((response: any) => {
                        const listCities = response.predictions.filter((city: any) => city.types?.includes('locality'));
