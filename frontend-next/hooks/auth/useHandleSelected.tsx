...
const useHandleSelected = (field: string, maxNum: number): [string[], OnSelectType] => {
  const [selecteds, setSelecteds] = useState<string[]>([]);

  const { setValue } = useFormContext();
  
  const onSetValueForm = useCallback((value: string[]) => {
  ...
  const onSelect = useCallback((itemSelected: string) => {
