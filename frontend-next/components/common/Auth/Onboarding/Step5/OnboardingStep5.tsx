...
const OnboardingStep5 = ({ onSubmit }: StepProps) => {
    ...
    const { data: filterCountries } = useSWR([`getCountryByKeyword`, countryKey], () =>
        getCountryByKeyword(countryKey || 'Japan'),
    );

    const { data: filterCities } = useSWR([`getCityByKeyword`, selectedCountry], () =>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col justify-between  px-10    ">
            <div className="flex flex-col h-[28rem]">
                ...
                <p className="text-base text-graySubtitle">
                    Connect with other like-minded individuals and organizations around you
                </p>
                <Combobox
                    label="Country"
